...
public class BilibiliSecurityHelper {
    ...
    private static String cipherPassword(@Nonnull BilibiliServiceProvider bilibiliServiceProvider,
        ...
        PublicKey publicKey;
        try {
            X509EncodedKeySpec x509EncodedKeySpec = new X509EncodedKeySpec(Base64.decode(rsaPublicKeyString.getBytes(), Base64.DEFAULT));
        ...
        try {
            ...
            cipheredPassword = new String(
                    Base64.encode(
                            cipher.doFinal((keyEntity.getData().getHash() + password).getBytes()), Base64.DEFAULT
                    )
            );
